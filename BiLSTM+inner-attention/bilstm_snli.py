...
class inner_attention(nn.Module):
    ...
    def __init__(self, encoder, p=0.25):
        ...
        self.WY = nn.Linear(2 * hidden_size, 2 * hidden_size) #论文中的W^y的权重 W^y * Y
        self.WH = nn.Linear(2 * hidden_size, 2 * hidden_size) #论文中的W^h的权重 W^h * R_ave
        self.W = nn.Linear(2 * hidden_size, 1) #论文中w^t w^t * M  M = tanh(W^y * Y + W^h * R_ave * e_L)
        self.fc1 = nn.Linear(8 * hidden_size, 300) # 8 * hidden_size , embbeding_size
